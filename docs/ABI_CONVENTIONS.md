...
| Opaque handles (future / Phase 4) | Opaque pointer typedef returned by an explicit `_create`, released by a matching `_destroy`. No handle is valid across a process boundary. |
...
| `nat_session_metadata_record_encode_json` / `_decode_json` | `ensureSessionMetadataRecordRegisteredForMetaRecord()` uses a **non-atomic** `static bool registered` check-then-set that mutates the function-local `metaRecordDecoders()` map on first use | **NOT thread-safe on first concurrent use.** Two threads racing the very first call can double-register / insert into the map concurrently. |

**Gap to close (tracked, not fixed in the Phase 1 change):** the
session-metadata registration guard should be made safe — e.g. `std::call_once`
or a Meyers-singleton that performs registration inside the initializer — before
these functions are called concurrently from multiple GIL-released Python
threads. Until then, treat `nat_session_metadata_record_*` as
single-threaded-first-call (call it once from the main thread during startup to
force registration before fanning out). This gap is easy to miss because it will
not reproduce in single-threaded testing.
